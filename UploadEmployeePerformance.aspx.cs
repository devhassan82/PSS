...
namespace PSS
{
    public partial class UploadEmployeePerformance : System.Web.UI.Page
    {
        ...
        private void Upload_Data()
        {
            ...
            //string _strFilePath = ConfigurationManager.AppSettings["FilePath"];
            string _strFilePath = Server.MapPath("~/Files/");
            string _strFileName = Path.GetFileName(fuDocument.FileName);
            string fileExtension = Path.GetExtension(fuDocument.FileName);
            _strFileName = _strFileName.Substring(0, _strFileName.LastIndexOf(Path.GetExtension(_strFileName))) + "_" + dtNow.ToString("yyyyMMddHHmmssfff") + Path.GetExtension(_strFileName);
            string excelPath = Path.Combine(_strFilePath, _strFileName);

            //string excelPath = Server.MapPath("~/Files/") + Path.GetFileName(fuDocument.PostedFile.FileName);
            fuDocument.SaveAs(excelPath);
            //string excelPath = Path.GetFileName(fuDocument.PostedFile.FileName);
        ...
        private void FillRow(int count, string EmployeeCode, string ProjectCode, int NoOfTasks, string RelatedObjective, int ProjectCompletion, int EfficiencyPerformance, ref DataTable dt)
        {
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            {
                //ErrorMessage(this.GetType().Name.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name.ToString(), ex.ToString());
                ErrorMessage(this.GetType().Name.ToString(), System.Reflection.MethodBase.GetCurrentMethod().Name.ToString(), ex.Message.ToString());
            }
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                Service.Service objSave = new Service.Service();
                clsStudentMarks obj = new clsStudentMarks();
                DataTable dtGrade = getXmlTable();
                dtGrade = BindGradeTable();
                if (ExamModeID == 1)
                {
                    obj.TypeID = 0;
                }
                else if (ExamModeID == 2)
                {
                    obj.TypeID = 1;
                }
                string _xml;
                DataSet dstb = new DataSet();
                dstb.Tables.Add(dtGrade);
                _xml = dstb.GetXml();

                //check If all rows are empty
                var rows = dtGrade.AsEnumerable().Where
                (row => row.Field<string>("OverAllmark") != String.Empty);//get the rows where the mark is not empty 
                string _success = "", message = "";
                if (rows.Any())
                {
                    obj.ExamMasterID = Convert.ToInt32(ExamMasterID);
                    obj.ModuleID = Convert.ToInt32(ModuleID);
                    obj.XML = _xml;
                    obj.CreatedBy = Convert.ToInt32(Session["UserID"].ToString());

                    string strData = Util.SerializeObject(obj, "clsStudentMarks");
                    string retval = objSave.GradeUpload_xml(strData, ref _success, ref message);
                    ...
                    {
                        message = "Unable to updated Student marks.";
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "DisplayCustomMessage", "DisplayCustomMessage('#alertBar','" + message + "','" + Constants.AlertType.Warning + "');", true);
                        System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "DisplayCustomMessage", "DisplayCustomMessage('#alertBar','" + message + "','" + Constants.AlertType.Warning + "');", true);
                    }
                }
                ...
                {
                    message = "Kindly input the grades. The mark column cannot remain empty.";
                    Page.ClientScript.RegisterStartupScript(this.GetType(), "DisplayCustomMessage", "DisplayCustomMessage('#alertBar','" + message + "','" + Constants.AlertType.Warning + "');", true);
                    System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "DisplayCustomMessage", "DisplayCustomMessage('#alertBar','" + message + "','" + Constants.AlertType.Warning + "');", true);
                }
            }
        ...
        */
        ...
        public static List<ModuleInfo> GetModule(string DepartmentID, string YearID, string SemesterID)
        {
            DataSet ds = new DataSet();
            ServiceReference1.Service1Client service = new ServiceReference1.Service1Client();
            //ds = obj.GetModuleForGrades(DepartmentID, YearID, SemesterID);
